...
object SampleArenaState {
    fun create(): ArenaViewState {
        ...
        return ArenaViewState(
            ...
            errors = 1,
            warnings = 4,
            ownCount = entities.count { it.kind in setOf(EntityKind.MainPlantation, EntityKind.OwnPlantation, EntityKind.IsolatedPlantation) },
            enemyCount = entities.count { it.kind == EntityKind.EnemyPlantation },
            constructionCount = entities.count { it.kind == EntityKind.Construction },
            beaverCount = entities.count { it.kind == EntityKind.BeaverLair },
            meteoCount = entities.count { it.kind in setOf(EntityKind.Sandstorm, EntityKind.Earthquake) },
            upgradePoints = 3,
            turnsUntilUpgrade = 26,
            cells = cells,
            entities = entities,
            alerts = alerts,
